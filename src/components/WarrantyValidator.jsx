...
const WarrantyValidator = ({ onValidationComplete, onClose }) => {
                ...
                {validationResult.warrantyStatus !== 'not_found' ? (
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="space-y-3">
                      ...
                      <div className="text-sm space-y-1">
                        VIN:</strong> {validationResult.vin}</p>
                        Model:</strong> {validationResult.model}</p>
                        Năm:</strong> {validationResult.year}</p>
                        Ngày mua:</strong> {new Date(validationResult.purchaseDate).toLocaleDateString('vi-VN')}</p>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <h4 className="font-medium">Thông tin khách hàng</h4>
                      <div className="text-sm space-y-1">
                        Tên:</strong> {validationResult.customer.name}</p>
                        SĐT:</strong> {validationResult.customer.phone}</p>
                        Email:</strong> {validationResult.customer.email}</p>
                      </div>
                    </div>
                    <div className="md:col-span-2 space-y-3">
                      ...
                      <div className="grid md:grid-cols-2 gap-4 text-sm">
                          ...
                          Bắt đầu:</strong> {new Date(validationResult.warrantyStartDate).toLocaleDateString('vi-VN')}</p>
                          Kết thúc:</strong> {new Date(validationResult.warrantyEndDate).toLocaleDateString('vi-VN')}</p>
                        </div>
                        
                          Km hiện tại:</strong> {validationResult.mileage.toLocaleString('vi-VN')} km</p>
                          Km tối đa:</strong> {validationResult.maxMileage.toLocaleString('vi-VN')} km</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">
                      Không tìm thấy thông tin xe với VIN: {validationResult.vin}</strong>
                    </p>
