...
const NewClaim = ({ onClose }: { onClose: () => void }) => {
  ...
  const handleWarrantyValidation = (vehicleInfo) => {
  ...
  const handleTechnicianAssignment = (assignment) => {
                ...
                <Card className={`max-w-2xl mx-auto ${
                  ...
                  <CardContent className="pt-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-4 text-sm">
                        ...
                        Xe:</strong> {claimData.vehicleInfo.model} ({claimData.vehicleInfo.year})</p>
                        VIN:</strong> {claimData.vehicleInfo.vin}</p>
                      </div>
                      
                        Khách hàng:</strong> {claimData.vehicleInfo.customer.name}</p>
                    ...
                    {claimData.vehicleInfo.warrantyStatus !== 'valid' && (
                      <div className="mt-4 p-3 bg-amber-100 border border-amber-300 rounded">
                        <p className="text-sm text-amber-800">
                          Gợi ý:</strong> Xe đã hết bảo hành. Có thể đề xuất khách hàng 
                  ...
                  <CardContent className="pt-6">
                    ...
                    <div className="text-sm space-y-3">
                      ...
                      {claimData.technicianAssignment.assistantTechnicians.length > 0 && (
                        ...
                        Thời gian ước tính:</strong> {claimData.technicianAssignment.estimatedTime}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ...
              <div className="grid gap-4">
                ...
                {claimData.vehicleInfo && (
                    ...
                    <CardContent className="grid md:grid-cols-2 gap-4 text-sm">
                        ...
                        Model:</strong> {claimData.vehicleInfo.model} ({claimData.vehicleInfo.year})</p>
                        VIN:</strong> {claimData.vehicleInfo.vin}</p>
                      </div>
                      
                        Khách hàng:</strong> {claimData.vehicleInfo.customer.name}</p>
                        Trạng thái bảo hành:</strong> 
                ...
                {/* Issue Info */}
                  ...
                  <CardContent className="text-sm space-y-2">
                    Loại sự cố:</strong> {claimData.issue}</p>
                    Mô tả:</strong> {claimData.description}</p>
                  </CardContent>
                </Card>
                ...
                {claimData.technicianAssignment && (
                    ...
                    <CardContent className="text-sm space-y-2">
                      Kỹ thuật viên chính:</strong> {claimData.technicianAssignment.mainTechnician.name}</p>
                      {claimData.technicianAssignment.assistantTechnicians.length > 0 && (
                        Hỗ trợ:</strong> {claimData.technicianAssignment.assistantTechnicians.map(t => t.name).join(', ')}</p>
                      )}
                      Thời gian ước tính:</strong> {claimData.technicianAssignment.estimatedTime}</p>
                    </CardContent>
                  </Card>
