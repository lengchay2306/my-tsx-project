...
const TechnicianWorkProgress = ({ 
  ...
  const handleTaskProgress = (taskId) => {
    onTaskUpdate(taskId, { 
      ...
      status,
      actualTime === 'completed' ? new Date().toLocaleTimeString('vi-VN') : undefined
    });
  ...
  const handleAddProgressNote = (taskId) => {
    ...
    const task = tasks.find(t => t.id === taskId);
    const updatedNotes = task?.notes ? `${task.notes}\n\n[${new Date().toLocaleString('vi-VN')}] ${progressUpdate}` ;
              ...
              <Card key={task.id} className={`transition-all ${
                ...
                <CardContent className="space-y-4">
                  <div className="grid md:grid-cols-2 gap-4 text-sm">
                      ...
                      Phân công:</strong> {task.assignedTo}</p>
                      Thời gian ước tính:</strong> {task.estimatedTime}</p>
                      {task.actualTime && (
                        Thời gian thực tế:</strong> {task.actualTime}</p>
                      )}
                    </div>
