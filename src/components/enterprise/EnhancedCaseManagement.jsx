...
const EnhancedCaseManagement = () => {
  ...
  const getPriorityBadge = (priority: "high" | "medium" | "low") => {
    const priorityConfig = {
      high: { variant: "destructive", text: "High", className: "bg-destructive/10 text-destructive" },
      medium: { variant: "warning", text: "Medium", className: "bg-warning/10 text-warning-foreground" },
      low: { variant: "secondary", text: "Low", className: "bg-muted text-muted-foreground" }
    };
  ...
  const filteredAndSortedCases = mockCases
    ...
    .sort((a, b) => {
      const aValue = a[sortField CaseData];
      const bValue = b[sortField CaseData];
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {/* Cases Table */}
          ...
          <div className="flex items-center justify-between">
            Cases ({filteredAndSortedCases.length})</CardTitle>
            <div className="flex items-center space-x-2 text-sm text-muted-foreground">
              Showing {filteredAndSortedCases.length} of {mockCases.length} cases</span>
            </div>
          </div>
        </CardHeader>
