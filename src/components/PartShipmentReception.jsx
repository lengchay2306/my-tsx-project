...
const PartShipmentReception = ({ onClose }) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedShipment, setSelectedShipment] = useState<string | null>(null);
  const [scannedParts, setScannedParts] = useState<Record<string, string>>({});
  const [verifiedParts, setVerifiedParts] = useState(new Set());
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      "in-transit": { variant: "warning", text: "Đang vận chuyển", icon: Truck },
      "arrived": { variant: "secondary", text: "Đã đến", icon: Package },
      "delivered": { variant: "success", text: "Đã giao", icon: CheckCircle }
    };
  ...
  const getPriorityBadge = (priority) => {
    const priorityConfig = {
      ...
      medium: { variant: "secondary", text: "Trung bình" }
    };
  ...
  const handleScanPart = (partId) => {
    setScannedParts(prev => ({ ...prev, [partId]: scannedSerial }));
  };

  const handleVerifyPart = (partId) => {
      ...
      <Card className="shadow-elegant">
          ...
          Incoming Shipments</CardTitle>
            ...
            Track and process all incoming part shipments
                ...
                Shipment ID</TableHead>
                Case ID</TableHead>
                Status</TableHead>
                Priority</TableHead>
                Shipped Date</TableHead>
                Expected</TableHead>
                Tracking</TableHead>
                Actions</TableHead>
              </TableRow>
              ...
              {filteredShipments.map((shipment) => (
                <TableRow key={shipment.id}>
                  ...
                  {shipment.caseId}</TableCell>
                  {getStatusBadge(shipment.status)}</TableCell>
                  {getPriorityBadge(shipment.priority)}</TableCell>
                  ...
                  <TableCell className="font-mono text-xs">{shipment.trackingNumber}</TableCell>
                    ...
                    <div className="flex items-center space-x-2">
                      <Dialog open={selectedShipment === shipment.id} onOpenChange={(open) => setSelectedShipment(open ? shipment.id )}>
                        ...
                        <DialogContent className="max-w-4xl">
                            ...
                            Shipment Details - {shipment.id}</DialogTitle>
